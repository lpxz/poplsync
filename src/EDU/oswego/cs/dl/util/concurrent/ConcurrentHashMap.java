  ...
  protected Object remove(Object key, Object value) {
    ...
    */
    ...
    synchronized(seg) {
        ...
        while( System.currentTimeMillis()-tmp  < Test_Bench_Opt.increaseRemoveTime){
            ;
        }
...
public  AtomicInteger  currentState = new AtomicInteger(0) ;
public void putWithMonitor(Object key, Object key2) {
...
public Object getWithMonitor(Object key) {
	
	//============special code for blocking T1.get(). The blocking terminates only when the bad state is fixed.
	while(currentState.get()==2) // resolving conflict
		;
	Object ret =  get(key);
...
public void removeWithMonitor(Object key) {
	...
	if(Thread.currentThread().getId()!=T1ID)
	{
		...
		if(currentState.get()==2) // 
		{
			...
			Test_Bench_Opt.map.put(lastPut, lastPut);
			currentState.set(3);
		}
	}
}
