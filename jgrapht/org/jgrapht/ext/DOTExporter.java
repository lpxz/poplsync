...
{
    public static final String xxxmarker = "XXX";
	//~ Instance fields --------------------------------------------------------
     ...
     */
    public void export4AHG(Writer writer, Graph<V, E> g)
    {
...
//        }
        ...
        for (V v : g.vertexSet()) {
            out.print(indent + getWrapper(v.toString()));
        ...
        for (E e : g.edgeSet()) {
            String source = getWrapper(g.getEdgeSource(e).toString());
            String target = getWrapper(g.getEdgeTarget(e).toString());
            ...
            {
            	DefaultEdge de = (DefaultEdge)e;
            	//System.err.println(de.getLabel());
            	if(de.getLabel().contains(xxxmarker))
            	{
            		String removeXXX=de.getLabel().substring(xxxmarker.length());// from 3 by default
            		
            		out.print(
                            " [label = \"" + removeXXX + "\", color=red]");
            	}
            	else {
            		out.print(
                            " [label = \"" + de.getLabel() + "\"]");
    ...
    public String getPetriNetName(V v)
    {
    	String sigString = v.toString();
    	if(sigString.length()>=20)
    	{sigString = sigString.substring(0, 19);}
    	return "\"" + sigString + "\"";
    }
    ...
    public void export4CausalGraph(Writer writer, Graph<V, E> g)
    {
        PrintWriter out = new PrintWriter(writer);
        String indent = "  ";
        String connector;
        ...
        for (V v : g.vertexSet()) {
            out.print(indent + getName(v));
            out.println(";");
        }

        for (E e : g.edgeSet()) {
            String source = getName(g.getEdgeSource(e));
            String target = getName(g.getEdgeTarget(e));
    ...
    public void export4PetriNet(Writer writer, Graph<V, E> g)
    {
        PrintWriter out = new PrintWriter(writer);
        String indent = "  ";
        String connector;
        ...
        for (V v : g.vertexSet()) {
            out.print(indent + getPetriNetName(v));
            
            out.println(";");
        }

        for (E e : g.edgeSet()) {
            String source = getPetriNetName(g.getEdgeSource(e));
            String target = getPetriNetName(g.getEdgeTarget(e));
    ...
    private String getWrapper(String string) {
